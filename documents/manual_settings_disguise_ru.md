# Руководство по настройке режима маскировки (Disguise Mode)

## Содержание

1. [Введение](#введение)
2. [Что такое режим маскировки](#что-такое-режим-маскировки)
...
Данное руководство описывает настройку режима маскировки (Disguise Mode) в приложении Camera ZeroDay. Режим маскировки позволяет скрыть приложение камеры за игрой 2048.

## Что такое режим маскировки

Режим маскировки — это функция безопасности, которая:
...
При включённом режиме маскировки:
...
Все настройки режима маскировки находятся в файле:
...
**Описание:** Главный переключатель режима маскировки.

| Значение | Эффект |
|----------|--------|
| `true`   | Режим маскировки **принудительно включён** для всех пользователей. Камера скрыта. Пользователи **не могут** отключить режим маскировки в настройках. |
| `false`  | Режим маскировки **опциональный**. Пользователи могут включать/выключать его в настройках приложения. |
...
// Пользователи сами решают, включать ли маскировку
DISGUISE_MODE: false,
```
...
1. Пользователь находится в игре 2048
2. Делает 3 быстрых нажатия (в течение 0.8 секунды)
...
### Пример 2: Опциональная маскировка
...
### Q: Можно ли полностью отключить режим маскировки?

**A:** Да, установите `DISGUISE_MODE: false`. Пользователи смогут управлять маскировкой через настройки приложения.

### Q: Сохраняются ли данные игры 2048?

**A:** Да, лучший результат сохраняется в localStorage браузера.

### Q: Работает ли маскировка на мобильных устройствах?
...
**A:** 
- Если `DISGUISE_MODE: true` — только через изменение `config.ts` и пересборку
- Если `DISGUISE_MODE: false` — пользователи могут изменить в настройках приложения (слайдер 1-30 минут)
